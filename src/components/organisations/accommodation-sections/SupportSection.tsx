...
interface SupportSectionProps {
  formData: IAccommodationFormData['SupportProvidedInfo'];
  onChange: (field: string, value: any) => void;
  errors: Record<string, string>;
}

export function SupportSection({ formData, onChange, errors }: SupportSectionProps) {
  const handleSupportToggle = (supportValue: SupportOfferedType) => {
    const currentSupport = formData.SupportOffered || [];
  ...
  return (
    <div className="space-y-4">
      <FormField label="Has On-Site Manager">
        <select
          value={formData.HasOnSiteManager ?? DiscretionaryValue.DontKnowAsk}
          ...
          {SUPPORT_OFFERED_OPTIONS.map((option) => (
            <div key={option.value} className="flex items-center">
              <input
                ...
                checked={formData.SupportOffered?.includes(option.value) || false}
      ...
      <FormField label="Support Information">
        <textarea
          ...
          rows={4}
          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          placeholder="Provide details about support services available"
        />
      </FormField>
    </div>
  );

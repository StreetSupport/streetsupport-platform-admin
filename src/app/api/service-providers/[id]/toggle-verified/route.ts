...
const patchHandler: AuthenticatedApiHandler = async (req: NextRequest, context, auth) => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/service-providers/${id}/toggle-verified`, {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ message: data?.message || 'Failed to toggle verified status' }, { status: response.status });
    }
